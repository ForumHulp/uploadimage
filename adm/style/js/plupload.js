...
phpbb.plupload.updateRow = function(index, downloadUrl) {
	...
	if (typeof downloadUrl !== 'undefined' && typeof downloadUrl[0] !== 'undefined') {
...
/**
 * Deleting a file removes it from the data
 *
 * @param {int} index	The index from phpbb.plupload.ids of the attachment to delete.
 */
phpbb.plupload.deleteData = function(index) {
	var datax = phpbb.plupload.data;
	...
	for (var i = 0; i < datax.length; i++) {
		if (i !== index)
		{
			phpbb.plupload.data[j] = datax[i];
			phpbb.plupload.ids.push(Number(datax[i].attach_id));
			j++;
		}
	}
};
...
phpbb.plupload.deleteFile = function(row, attachId) {
	...
	var fields = {};
	fields['delete_file[' + index + ']'] = index; //phpbb.plupload.data[index].real_filename;
	...
	var done = function(response) {
		...
		phpbb.plupload.deleteData(index);
